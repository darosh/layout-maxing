// `save-presets`: writes Best (for ex1-4) and BestClustered (for ex5) preset files
...
async function patchIndex(flags: { best: boolean; bestClustered: boolean; bestStable: boolean; bestStableCluster: boolean }) {
  ...
  if (flags.bestClustered) {
    text = addImport(text, "import Clustered from './clustered.ts'", "import BestClustered from './best-clustered.ts'")
    text = addExport(text, '  Clustered,\n', '  BestClustered,\n')
  }
  if (flags.bestStableCluster) {
    text = addImport(text, "import BestClustered from './best-clustered.ts'", "import BestStableCluster from './best-stable-cluster.ts'")
    text = addExport(text, '  BestClustered,\n', '  BestStableCluster,\n')
  }
...
export async function savePresets(db: BenchDb): Promise<{
  ...
  bestClustered?: { runId: number; score: number }
  ...
  if (ex5Row) {
    ...
    await Deno.writeTextFile(resolve(PRESETS_DIR, 'best-clustered.ts'), renderPreset('BestClustered', delta, header))
    result.bestClustered = { runId: ex5Row.id, score: ex5Row.score_default ?? Infinity }
  }
  ...
  await patchIndex({
    best: !!result.best,
    bestClustered: !!result.bestClustered,
